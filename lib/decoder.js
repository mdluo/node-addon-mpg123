...
function Decoder (opts) {
  ...
  if (opts && (opts.sampleRate || opts.channels || opts.float || opts.bitDepth || opts.signed)) {
    ...
    if (opts.float) {
      if (opts.bitDepth != null) {
        encoding = binding['MPG123_ENC_FLOAT_' + opts.bitDepth];
      } else {
        encoding = binding.MPG123_ENC_FLOAT_32 | binding.MPG123_ENC_FLOAT_64;
      }
    } else {
      if (opts.bitDepth != null) {
        encoding = binding['MPG123_ENC_' + (opts.signed ? 'SIGNED_' : 'UNSIGNED_') + opts.bitDepth];
      } else if (opts.signed) {
        encoding = binding.MPG123_ENC_SIGNED_8 | binding.MPG123_ENC_SIGNED_16 | binding.MPG123_ENC_SIGNED_24 | binding.MPG123_ENC_SIGNED_32;
      } else {
        encoding = binding.MPG123_ENC_UNSIGNED_8 | binding.MPG123_ENC_UNSIGNED_16 | binding.MPG123_ENC_UNSIGNED_24 | binding.MPG123_ENC_UNSIGNED_32;
      }
    }
    if (encoding == null) {
